...
def generate_sales_entry(self, tenant_schema: str, invoice_id):
...
@shared_task(bind=True, max_retries=3, default_retry_delay=30)
def generate_purchase_entry(self, tenant_schema: str, bill_id):
...
@shared_task(bind=True, max_retries=3, default_retry_delay=30)
def generate_payment_entry(self, tenant_schema: str, payment_id):
...
@shared_task(bind=True, max_retries=3, default_retry_delay=30)
def generate_payroll_entry(self, tenant_schema: str, payroll_id):
...
@shared_task(bind=True, max_retries=3, default_retry_delay=30)
def generate_inventory_entry(self, tenant_schema: str, adjustment_id):
...
def process_recurring_entries(self, tenant_schema: str):
    """
    Process all due recurring journal entries for one tenant.
    ...
    try:
        ...
        with schema_context(tenant_schema):
            ...
            logger.info("Processed %d recurring entries for schema=%s", len(entries), tenant_schema)
            return len(entries)
    except Exception as exc:
        logger.exception("Failed to process recurring entries for schema=%s", tenant_schema)
        raise self.retry(exc=exc)


@shared_task(bind=True, max_retries=3, default_retry_delay=60)
def generate_scheduled_report(self, tenant_schema: str, config_id):
    """
...
@shared_task(bind=True, max_retries=3, default_retry_delay=60)
def check_tax_filing_deadlines(self, tenant_schema: str):
    """Daily task: check pending tax filings and send reminders for one tenant.
    ...
    logger.info("Starting tax filing deadline check for schema=%s …", tenant_schema)
    ...
    logger.info("Tax deadline check done for schema=%s — %d reminders sent.", tenant_schema, reminders_sent)
    return {
