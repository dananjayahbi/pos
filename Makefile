...
.PHONY: help up down build rebuild logs logs-backend logs-frontend restart ps \
        dev shell dbshell migrate makemigrations createsuperuser collectstatic \
        test test-backend test-frontend coverage \
        lint lint-backend lint-frontend format format-check \
        clean seed backup restore \
